...
try:
    ...
    st.error(
        "Error: 'rna_utils.py' not found. Please ensure it is in the same directory."
    )
    st.stop()
...
def run_training(pdb_dir, out_dir, atom_type, max_dist, bin_width):
    ...
    status_container.update(
        label="Calculating statistical potentials...", state="running"
    )
    EPS = 1e-12
...
def run_scoring(pdb_path, pot_dir):
    try:
        ...
        return None, None, f"Failed to load params.txt: {e}"
        ...
        with tab_train:
            ...
            with col1:
                ...
                atom_type = st.selectbox(
                    ...
                    atom_options,
                    index=0,
                    help="Select the atom used for distance calculations",
                )
            ...
            with col2:
                ...
                if file_count > 0:
                    ...
                    st.warning(
                        "No training files found. Please go to the 'Welcome' page to deposit files."
                    )

...
lottie_rna = load_lottieurl("https://lottie.host/6e6403c9-0d6c-4aa7-933e-0014b2d499fa/R5Xv6p6XyK.json")
...
def run_plotting_engine(pot_dir, plot_type="Combined Overlay"):
    ...
    if plot_type == "Heatmap Matrix":
        ...
        fig = px.imshow(
            ...
            zmin=-4, zmax=4, # Clamp for visual contrast
            aspect="auto"
        )
        fig.update_layout(title=f"Potential Landscape ({atom})", template="plotly_dark", height=600)
        return fig, None
    ...
    # --- Mode 2: Combined Overlay ---
        ...
        fig.update_layout(
            ...
            xaxis_title="Distance (Å)", yaxis_title="Pseudo-energy (kT)",
            template="plotly_dark", # UPDATED FOR DARK THEME
            hovermode="x unified", height=600,
            legend=dict(yanchor="top", y=0.99, xanchor="left", x=1.01),
        )
        fig.add_shape(type="line", x0=0, y0=0, x1=max_dist, y1=0, line=dict(color="white", width=1, dash="dash"))
    ...
    # --- Mode 3: Grid View ---
        ...
        for idx, pair in enumerate(PAIR_TYPES):
            ...
            fig.add_shape(type="line", x0=0, y0=0, x1=max_dist, y1=0,
                line=dict(color="white", width=1, dash="dash"), row=row, col=col)
        fig.update_layout(title=f"Statistical Potentials ({atom}) - Grid View", template="plotly_dark", height=700)
        fig.update_yaxes(range=[-11, 11])
...
def main():
    ...
    if nav_selection == "Welcome":
        ...
        if uploaded_files:
            ...
            progress_text.empty()
            if error_count > 0:
                st.warning(f"Cached {len(saved_files)} files. (Skipped {error_count} problematic files)")
            else:
            ...
            st.info(f"{st.session_state['training_file_count']} files currently in memory.")
    ...
    # --- PAGE: PIPELINE DASHBOARD ---
        ...
        # 1. CONFIGURATION
        with st.expander("Configuration", expanded=not st.session_state["pipeline_run"]):
        ...
        if run_btn and st.session_state["training_file_count"] > 0:
            ...
            if success:
                ...
                status_box.update(label="Pipeline Completed", state="complete", expanded=False)
                st.rerun()
            else:
        ...
        if st.session_state["pipeline_run"] and st.session_state["potentials_dir"]:
            tab_viz, tab_score, tab_download = st.tabs(["Visualisation", "Scoring", "Export"])
            ...
            with tab_viz:
                ...
                with c_sel:
                    ...
                    plot_choice = st.selectbox("View Mode", ["Combined Overlay", "Grid View", "Heatmap Matrix"])
            ...
            with tab_score:
                ...
                with col_left:
                    ...
                    target_pdb = st.file_uploader("Target PDB", type="pdb", label_visibility="collapsed")
                    score_trigger = st.button("Calculate Free Energy", disabled=not target_pdb)
                ...
                with col_right:
                    ...
                    if score_trigger and target_pdb:
                        ...
                        if err:
                            ...
                            # Color logic
                            if score < -10: score_label = "Favourable"
                            elif score > 5: score_label = "Unfavourable"
                            else: score_label = "Neutral"
                            
                            c1, c2 = st.columns(2)
                            c1.metric("Pseudo-Energy", f"{score:.4f}")
                            c2.metric("Interactions", pairs)
                            
                            if score < 0: st.success(score_label)
                            else: st.warning(score_label)
        ...
        elif not st.session_state["pipeline_run"] and st.session_state["training_file_count"] > 0:

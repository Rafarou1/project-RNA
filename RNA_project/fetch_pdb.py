...
def get_latest_rna_structures(limit=50):
    ...
    try:
        data_response = requests.post(
            data_url, json={"query": graphql_query, "variables": variables}
        )
    ...
    for entry in entries:
        ...
        formatted_results.append({"id": pdb_id, "date": date_display, "title": title})
...
if __name__ == "__main__":
    ...
    print(
        f"Fetching latest {SEARCH_BUFFER} RNA candidates from PDB to find {TARGET_NEW_DOWNLOADS} new .pdb files...\n"
    )
    ...
    if candidates:
        ...
        download_only_pdb_structures(
            candidates, target_directory, target_count=TARGET_NEW_DOWNLOADS
        )
    else:
